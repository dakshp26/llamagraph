...
def validate_graph(payload: GraphPayload) -> list[GraphValidationIssue]:
    ...
    for n in nodes:
        ...
        if t == "json_api":
            ...
            has_placeholders = bool(_PLACEHOLDER_RE.search(all_text))
            if has_placeholders and not incoming[n.id]:
                errors.append(
                    GraphValidationIssue(
                        n.id,
            ...
            if not url:
                ...
                target_handles = {e.target_handle for e in incoming[n.id] if e.target_handle}
                for name in _PLACEHOLDER_RE.findall(all_text):
